...
--
  Utilisez le patch suivant :
--


Monitorez cette log dans ELK.

Utilisez ces commandes pour lancer les conteneurs Elastic et Kibana
----
docker network create elastic
docker run --network=elastic -e ELASTIC_PASSWORD=MagicWord -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" docker.elastic.co/elasticsearch/elasticsearch-platinum:6.2.3
...
== Mettre en place une authent forte
